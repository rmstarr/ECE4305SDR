function [cfgLLAdv,pktCnt,crcCnt,startIdx,payload] = ...
                    helperBLEPhyBitRecover_Data_No_Time_Comp(rcv,pktCnt,crcCnt,bleParam)
...
%     if length(syncFrame) >= bleParam.MinimumPacketLen
        ...
        % GMSK demodulation
        demodBits = ble.internal.gmskdemod(rcv,bleParam.SamplesPerSymbol);
        ...
        if isequal(rcvPreamble,bleParam.Preamble)
            ...
            if isequal(accAddress,bleParam.AccessAddress)
                ...
                if length(dewhitenedBits) >= PDULenInBits+bleParam.CRCLength+bleParam.HeaderLength
                    ...
                    [status, cfgLLAdv, payload] = bleLLDataChannelPDUDecode(pduBitsWithCRC);
