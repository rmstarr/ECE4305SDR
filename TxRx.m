...
frameSize = 2^5;                       % Frame size
numFrames = 15;                        % Total frames to simulate
...
tscope = dsp.TimeScope('YLabel', 'EVM (%)', 'YLimits', [0 40], 'SampleRate', 1000, 'TimeSpan', 1.2, 'ShowGrid', true);
...
while frameCount < numFrames
    ...
    rxFilterEVM = evm(filteredRxData);
    rxTCorrectedEVM = evm(tCorrectedSig);
    
    %tscope(rxTCorrectedEVM);
