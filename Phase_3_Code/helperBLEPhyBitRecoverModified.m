function [cfgLLData,pktCnt,crcCnt,startIdx, payload] = ...
                    helperBLEPhyBitRecoverModified(rcv,prbIdx,pktCnt,crcCnt,bleParam)
        ...
        if isequal(rcvPreamble,bleParam.Preamble)
            ...
            if isequal(accAddress,bleParam.AccessAddress)
                ...
                if length(dewhitenedBits) >= PDULenInBits+bleParam.CRCLength+bleParam.HeaderLength
                    ...
                    [status, cfgLLData, payload] = bleLLDataChannelPDUDecode(pduBitsWithCRC, bleParam.CRC);
