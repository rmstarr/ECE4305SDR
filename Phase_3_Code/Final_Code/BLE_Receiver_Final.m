...
while length(dataCaptures) > bleParam.MinimumPacketLen
    ...
    [cfgLLData,pktCnt,crcCnt,remStartIdx, payload] = blePhyBitRecover(rcvFilt,...
    ...
    % Need to handle uneven packet sizes or lost data
    if leftovers ~= 0 && isempty(payload) == true
    ...
    if skip == 0
        ...
        % Only use for sensor data stuff
        [rows, cols] = size(decodedData2);
        decodedData2 = reshape(decodedData2, rows/2, 2);
        if displayFlag && ~isempty(cfgLLData)
            minutesMax = (rows/2)-1;
            tStamps = datetime('now')-minutes(minutesMax):minutes(1):datetime('now');
            disp('Decoded Data:')
            disp(decodedData2);
        end
